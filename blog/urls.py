...
urlpatterns = [
    ...
    # Example: /2012/11/
    url(r'^(?P<year>\d{4})/(?P<month>\d{2})/$', PostMAV.as_view(month_format='%m'), name='post_month_archive'),

    # Example: /2012/11/10/
    url(r'^(?P<year>\d{4})/(?P<month>\d{2})/(?P<day>\d{2})/$', PostDAV.as_view(month_format='%m'), name='post_day_archive'),
